...
### 2. 检查 AGENT_PROMPT.md

```bash
grep -i "worktree.*director" AGENT_PROMPT.md 2>/dev/null
```
...
如果没有现有目录且 AGENT_PROMPT.md 中无偏好设置：
...
- **问题：** 造成不一致，违反项目约定
- **修复：** 遵循优先级：现有目录 > AGENT_PROMPT.md > 询问
...
- 不询问就带着失败的测试继续
- 在有歧义时假设目录位置
- 跳过 AGENT_PROMPT.md 检查

**始终：**
- 遵循目录优先级：现有目录 > AGENT_PROMPT.md > 询问
- 对项目本地目录验证是否已忽略
- 自动检测并运行项目设置
- 验证测试基线干净
