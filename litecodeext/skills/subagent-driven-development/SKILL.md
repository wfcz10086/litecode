...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "在 save_memory 中标记任务完成" [shape=box];
    }

    "读取计划，提取所有任务的完整文本，记录上下文，创建 save_memory" [shape=box];
    ...
    "读取计划，提取所有任务的完整文本，记录上下文，创建 save_memory" -> "分派实现子智能体 (./implementer-prompt.md)";
    ...
    "代码质量审查子智能体通过?" -> "在 save_memory 中标记任务完成" [label="是"];
    "在 save_memory 中标记任务完成" -> "还有剩余任务?";
...
[一次性读取计划文件：docs/feature-plan.md]
[提取全部 5 个任务的完整文本和上下文]
[用所有任务创建 save_memory]
