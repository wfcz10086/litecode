...
```typescript
// 在 LiteCode / AI 环境中
spawn_agent("修复 agent-tool-abort.test.ts 的失败")
spawn_agent("修复 batch-completion-behavior.test.ts 的失败")
spawn_agent("修复 tool-approval-race-conditions.test.ts 的失败")
// 三个任务并发运行
```
